...
{
    bool ret = Processor::getAllGroupMessage(mInfo.id, localConfirmId);
    if (!ret) {
...
//            bigNum = max(bigNum, mUnReadMessageList[i]->id);
    ...
    if (!mUnReadMessageList.empty()) {
        emit confirmMessage(mInfo.id, ::user_id, localConfirmId, confirmVec);//网络发送给服务器确认收到【start，end】的消息
    }
}
...
void GroupChatWindow::messageUpdate(MessageInfo* info) {
    ...
    emit confirmMessage(mUserId, ::user_id, localConfirmId, vec);//网络发送给服务器确认收到【start，end】的消息
}
...
void GroupChatWindow::ProcessGroupMessageSuccess(Response response) {
    ...
    if (groupId == mInfo.id && user_id == userId) {
        localConfirmId = conId;
        //TODO:写本地配置文件
        qDebug() << "flush localConfirmId : " << localConfirmId;
    }
}
...
int GroupChatWindow::getGroupLocalConfirmId()
{
    ...
    QString groupItem = "group/" + QString::fromStdString(mInfo.group_name) + "/id";
    int confirmId = settings.value(groupItem).toInt();
    localConfirmId = confirmId;
    return localConfirmId;
}
